...
const TrendChart = React.memo((props) => {
  ...
  const convertToMonthsView = (arr) => {
    ...
    for (const item of vals) {
      ...
      const dateMonth = new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), 1));
  ...
  const transformData = (dataJSON, transformMode, transformProps) => {
    ...
    if (transformMode === "base") {
      ...
      for (const key in finVals) {
        ...
        const yearago = new Date(Date.UTC(date.getUTCFullYear() - 1, date.getUTCMonth(), 1));
        const yearagokey = String(yearago.getTime());
        if (yearagokey in finVals) {
          yoyChange[key] = 100*(finVals[key] - finVals[yearagokey])/finVals[yearagokey];
        }
      }
      return Object.entries(yoyChange).sort((a, b) => {return a[0] - b [0]}).map((el) => [parseInt(el[0]), el[1]]);
      ...
      for (const key in finVals) {
        ...
        if (date.getUTCMonth() !== 0) {
          monthago = new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth() - 1, 1));
        } else {
          monthago = new Date(Date.UTC(date.getUTCFullYear() - 1, 11, 1));
        }
        const monthagokey = String(monthago.getTime());
        if (monthagokey in finVals) {
          momChange[key] = 100*(finVals[key] - finVals[monthagokey])/finVals[monthagokey];
        }
      }
      return Object.entries(momChange).sort((a, b) => {return a[0] - b [0]}).map((el) => [parseInt(el[0]), el[1]]);
      ...
      return Object.entries(momChange).sort((a, b) => {return a[0] - b [0]}).map((el) => [parseInt(el[0]), el[1]]);
    } else if (transformMode === "rollingMean" || transformMode === "rollingSum") {
      const originalData = [...vals];
      const finVals = [];
      const rollingWindow = transformProps?.rollingWindow;
      for (let idx = rollingWindow; idx < originalData.length + 1; idx++) {
        const windowedArray = [...originalData].slice(idx - rollingWindow, idx).map((el) => el[1]);
  ...
  if (data?.dataJSON instanceof Array) {
    ...
    seriesToPlot = [
      {
        ...
        data: transformData(data.dataJSON || [], props.transformMode, props.transformProps),
        color: {
          linearGradient: { x1: 0, x2: 0, y1: 0, y2: 1 },
          stops: [
            [
              0,
              "rgba(67, 104, 247, 0.05)",
            ],
            //this controls how quickly the gradient deepens
            [
              0.2,
              "rgba(67, 104, 247, 0.075)",
            ], //this controls how quickly the gradient deepens
            [
              1,
              "rgba(67, 104, 247, 0.002)",
            ],
          ],
        },
        lineWidth: 0,
    ...
    yaxis = {
      title: {
        ...
        style: { fontSize: "10px", fontFamily: props.style?.titleFont || "Inter", color: props.style?.axisColor || undefined },
      },
      labels: {
        style: {
          fontSize: String(props.style?.axisLabelSize) || 14 + "px",
    ...
    if (data?.compare === true) {
      yaxis = {...yaxis, labels: {...yaxis.labels, formatter: function() {return (this.value > 0 ? " + " : "") + this.value + "%";}}}
    } else {
      yaxis = {...yaxis, labels: {...yaxis.labels, formatter: undefined}}
    }
  }
  ...
  const options = {
    chart: {
      ...
      zoomType: "x",
      animation: false,
      height: props.height !== "undefined" && props.height ? props.height : 280,
      panning: false,
    },
